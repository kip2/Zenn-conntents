...
これは単純な解決方法で、階層が深くなることでrust-analyzerが働かないなら、階層を低くしちゃえばいいじゃない、という方法。
...
なお、パスは何個でも記載が可能になので、管理するCargoプロジェクトが増えても安心。
...
これで該当のディレクトリでのrust-analyzerのバージョンが固定できたので、補完が効くようになる。

## 終わりに

いままで同様の問題が発生しては放置してたのを、今回思い切って解消して記事にまとめてみました。
